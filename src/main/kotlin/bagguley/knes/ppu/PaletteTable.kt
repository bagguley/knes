...
class PaletteTable {
    ...
    var emphTable:Array<IntArray> = Array(8) { IntArray(64) }
    var currentEmph = -1
    ...
    fun makeTables() {
        ...
        for (emph in 0 until 8) {
            ...
            //emphTable[emph] = IntArray(64)

            // Calculate table:
            for (i in 0 until 64) {
                val col = curTable[i]
                val r = floor(getRed(col) * rFactor).toInt()
                val g = floor(getGreen(col) * gFactor).toInt()
                val b = floor(getBlue(col) * bFactor).toInt()
